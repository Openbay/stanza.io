...
function unescapeTags(text, tagNames) {
    var unescapedText;
    for (var i = 0; i < tagNames.length; i++) {
        var tagName = tagNames[i];
        var openTagRegex = new RegExp(`&lt;${tagName}(.*?)&gt;`, 'g');
        var closeTagRegex = new RegExp(`&lt;/${tagName}&gt;`, 'g');
        var openTag = `<${tagName}$1>`;
        var closeTag = `</${tagName}>`;
        unescapedText = (unescapedText || text).replace(openTagRegex, openTag).replace(closeTagRegex, closeTag);
    }
    return unescapedText;
}

function addFormattingToBody(text) {
    return text.replace(/<body>(?!.*<body>)([^]*?)<\/body>/, "<body>$1</body><html xmlns='http://jabber.org/protocol/xhtml-im'><body xmlns='http://www.w3.org/1999/xhtml'><p>$1</p></body></html>");
}
...
BOSHConnection.prototype.request = function (bosh) {
    ...
    var brokenBody = newlinesToBreaks(bosh.toString());
    var unescapedBody = unescapeTags(brokenBody, ['b', 'i', 'u', 'span']);
    var formattedBody = addFormattingToBody(unescapedBody);

    var body = new Buffer(formattedBody, 'utf8').toString();
